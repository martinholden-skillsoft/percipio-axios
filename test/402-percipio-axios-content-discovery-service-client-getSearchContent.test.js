...
describe(' PercipioAxiosContentDiscoveryServiceClient', function () {
  describe('getSearchContent function', function () {
    let mainconfig;
    let requestConfig;
    let requestNock;
    ...
    beforeEach(function () {
      ...
      requestNock = testutils.getOperationNock(
        mainconfig.baseURL,
        '/content-discovery/v1/organizations/{orgId}/search-content',
    ...
    it('confirm it has default headers', async function () {
      ...
      return client.getSearchContent(requestConfig).then((response) => {
    ...
    it('confirm default params sent', async function () {
      const client = new PercipioAxiosContentDiscoveryServiceClient(mainconfig);

      return client.getSearchContent(requestConfig).then((response) => {
        const requestQS = testutils.getAxiosRequestQuerystring(response);
        expect(requestQS).to.have.property('offset', 0);
        expect(requestQS).to.have.property('max', 25);
      });
    });

    it('confirm supported params override defaults', async function () {
      const client = new PercipioAxiosContentDiscoveryServiceClient(mainconfig);
      ...
      requestConfig.params = {
        ...
        typeFilter: 'override',
        modality: 'override',
        expertiseLevel: 'override',
        licensePoolIds: 'override',
        offset: 'override',
        max: 'override',
      };
      return client.getSearchContent(requestConfig).then((response) => {
        const requestQS = testutils.getAxiosRequestQuerystring(response);
        expect(requestQS).to.have.property('q', 'override');
        expect(requestQS).to.have.property('localeCode', 'override');
        expect(requestQS).to.have.property('typeFilter', 'override');
        expect(requestQS).to.have.property('modality', 'override');
        expect(requestQS).to.have.property('expertiseLevel', 'override');
        expect(requestQS).to.have.property('licensePoolIds', 'override');
        expect(requestQS).to.have.property('offset', 'override');
        expect(requestQS).to.have.property('max', 'override');
      });
    });

    it('confirm unsupported params not sent', async function () {
      const client = new PercipioAxiosContentDiscoveryServiceClient(mainconfig);

      requestConfig.params = { a: 1, b: 2, c: 'mystring' };
      return client.getSearchContent(requestConfig).then((response) => {
        const requestQS = testutils.getAxiosRequestQuerystring(response);
        expect(requestQS).to.have.property('offset', 0);
        expect(requestQS).to.have.property('max', 25);
        expect(requestQS).to.not.have.property('a', 1);
        expect(requestQS).to.not.have.property('b', 2);
        expect(requestQS).to.not.have.property('c', 'mystring');
      });
    });
    ...
    it('confirm data not sent', async function () {
      const client = new PercipioAxiosContentDiscoveryServiceClient(mainconfig);

      requestConfig.data = { a: 1, b: 2, c: 'mystring' };
      return client.getSearchContent(requestConfig).then((response) => {
        expect(response.request).to.have.property('requestBodyBuffers');
        expect(testutils.isAxiosRequestBodyEmpty(response)).to.be.true;
      });
    });
  });
});
