...
describe('PercipioAxiosReportingServicesClient', function () {
  describe('constructors', function () {
    ...
    it('throws PropertyRequiredError if baseURL is not supplied', function () {
      ...
      expect(() => new PercipioAxiosReportingServicesClient(config)).to.throw(
        PropertyRequiredError
      );
    });
    ...
    it('throws PropertyInvalidError if baseURL is not url', function () {
      ...
      expect(() => new PercipioAxiosReportingServicesClient(config)).to.throw(PropertyInvalidError);
    });
    ...
    it('creates PercipioAxiosReportingServicesClient', function () {
      const client = new PercipioAxiosReportingServicesClient(mainconfig);
      expect(client).to.be.instanceof(PercipioAxiosReportingServicesClient);
      ...
      expect(client.requestLearningActivityReport).to.be.instanceOf(
        Function,
        'requestLearningActivityReport is not a function of client'
      );
      expect(client.requestContentActivityReport).to.be.instanceOf(
        Function,
        'requestContentActivityReport is not a function of client'
      );
      expect(client.requestEntitlementReport).to.be.instanceOf(
        Function,
        'requestEntitlementReport is not a function of client'
      );
      expect(client.requestCollectionsContentReport).to.be.instanceOf(
        Function,
        'requestCollectionsContentReport is not a function of client'
      );
      expect(client.getReportRequest).to.be.instanceOf(
        Function,
        'getReportRequest is not a function of client'
      );
    });
  });
});
