...
describe('PercipioAxiosClient', function () {
  describe('sendRequest function', function () {
    ...
    it('invoke sendRequest confirm it has default headers - get', async function () {
    ...
    it('invoke sendRequest confirm it has default headers - post', async function () {
      const client = new PercipioAxiosClient(mainconfig);

      getRequestConfig.method = 'post';
      return client.sendRequest(getRequestConfig).then((response) => {
        expect(requestNock).to.have.been.requestedWithHeadersMatch({
          authorization: `Bearer ${mainconfig.bearer}`,
          'content-type': 'application/json',
    ...
    it('invoke sendRequest confirm it has default headers - patch', async function () {
      const client = new PercipioAxiosClient(mainconfig);

      getRequestConfig.method = 'patch';
      return client.sendRequest(getRequestConfig).then((response) => {
        expect(requestNock).to.have.been.requestedWithHeadersMatch({
          authorization: `Bearer ${mainconfig.bearer}`,
          'content-type': 'application/json',
    ...
    it('invoke sendRequest confirm response contains timing data - get', async function () {
      const client = new PercipioAxiosClient(mainconfig);
    ...
    it('invoke sendRequest with custom axios instance without timingAdapter to confirm response does not contain timing data - get', async function () {
    ...
    it('invoke sendRequest confirm querystring params sent - get', async function () {
      const client = new PercipioAxiosClient(mainconfig);

      getRequestConfig.query = { a: 1, b: 2, c: 'mystring' };
      return client.sendRequest(getRequestConfig).then((response) => {
    ...
    it('invoke sendRequest confirm body sent - get', async function () {
      const client = new PercipioAxiosClient(mainconfig);

      getRequestConfig.body = { a: 1, b: 2, c: 'mystring' };
      return client.sendRequest(getRequestConfig).then((response) => {
    ...
    it('throws PropertyRequiredError if method is not included', function () {
      const client = new PercipioAxiosClient(mainconfig);
      // remove method from config
      getRequestConfig.method = null;
