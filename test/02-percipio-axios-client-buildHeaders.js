...
describe('PercipioAxiosClient', function () {
  describe('buildHeaders function', function () {
    ...
    const overrideHeaders = {
      ...
      Authorization: 'Bearer Authorization-Override',
    };
    ...
    it('returns default headers and extra headers', function () {
      const client = new PercipioAxiosClient(mainconfig);
      const result = client.buildHeaders(headers);

      expect(result).to.have.property('content-type', 'application/json');
      expect(result).to.have.property('authorization', `Bearer ${mainconfig.bearer}`);
      expect(result).to.have.property('extraheader1', headers.ExtraHeader1);
      expect(result).to.have.property('extraheader2', headers.ExtraHeader2);
    });
    ...
    it('throws PropertyInvalidError if overide of authorization header is not Bearer authentication', function () {
      const client = new PercipioAxiosClient(mainconfig);

      expect(() => client.buildHeaders(badAuthHeaders)).to.throw(errors.PropertyInvalidError);
    });
  });
});
