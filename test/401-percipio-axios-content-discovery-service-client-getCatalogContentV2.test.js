...
describe(' PercipioAxiosContentDiscoveryServiceClient', function () {
  describe('getCatalogContentV2 function', function () {
    let mainconfig;
    let requestConfig;
    let requestNock;
    ...
    beforeEach(function () {
      ...
      requestNock = testutils.getOperationNock(
        mainconfig.baseURL,
        '/content-discovery/v2/organizations/{orgId}/catalog-content',
    ...
    it('confirm it has default headers', async function () {
      ...
      return client.getCatalogContentV2(requestConfig).then((response) => {
    ...
    it('confirm default params sent', async function () {
      ...
      return client.getCatalogContentV2(requestConfig).then((response) => {
        ...
        expect(requestQS).to.have.property('max', 1000);
      });
    });
    ...
    it('confirm supported params override defaults', async function () {
      ...
      requestConfig.params = {
        transformName: 'override',
        updatedSince: 'override',
        offset: 'override',
        max: 'override',
        system: 'override',
        pagingRequestId: 'override',
      };
      return client.getCatalogContentV2(requestConfig).then((response) => {
        const requestQS = testutils.getAxiosRequestQuerystring(response);
        expect(requestQS).to.have.property('transformName', 'override');
        expect(requestQS).to.have.property('updatedSince', 'override');
        expect(requestQS).to.have.property('offset', 'override');
        expect(requestQS).to.have.property('max', 'override');
        expect(requestQS).to.have.property('system', 'override');
        expect(requestQS).to.have.property('pagingRequestId', 'override');
      });
    });
