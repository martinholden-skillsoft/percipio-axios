 ...
 * ### .param
 *
 * Assert that an Axios `Request` object path has a query string parameter with a given
 ...
 *
 * ```js
 * expect(req).to.have.body('orderby');
 * expect(req).to.have.body('orderby', 'date');
 * expect(req).to.not.have.body('limit');
 * ```
 *
 * @param {String} parameter name
 * @param {String} parameter value
...
Assertion.addMethod('body', function (name, value) {
  var assertion = new Assertion();
  chai.util.transferFlags(this, assertion);
...
describe('axiosExtras', function () {
  describe('timingAdapter', function () {
    ...
    beforeEach(function () {
      ...
      setupConfigs();
      requestNock = nock(mainconfig.baseUrl)
        .get(`/test/${mainconfig.orgId}/path`)
        .query(true)
        .reply(200, {})
        .patch(`/test/${mainconfig.orgId}/path`)
        .query(true)
        .reply(200, {})
        .post(`/test/${mainconfig.orgId}/path`)
        .query(true)
        .reply(200, {})
        .head(`/test/${mainconfig.orgId}/path`)
        .query(true)
        .reply(200, {})
        .delete(`/test/${mainconfig.orgId}/path`)
        .query(true)
        .reply(200, {});
    });
